...
# ================== 自定义端口 ==================
echo -e "${CYAN}${BOLD}请输入 Xray 监听端口 [留空直接回车 = 随机2000-9000]：${NC}"
read -r CUSTOM_PORT
if [[ -z "$CUSTOM_PORT" ]]; then
    PORT=$((RANDOM % 7001 + 2000))
    echo -e "${YELLOW}→ 未输入，使用随机端口：${PORT}${NC}"
else
    PORT=$CUSTOM_PORT
...
show_banner() {
    ...
    echo -e "${CYAN}${BOLD}║                                                                              ║${NC}"
    echo -e "${CYAN}${BOLD}║               ${WHITE}Dack TG@Duck678 -高性能代理服务器一键部署脚本${CYAN}${BOLD}                             ║${NC}"
...
ensure_ssh_port_open() {
    ...
    if [[ -z "$CUSTOM_SSH" ]]; then
        SSH_PORT=$CURRENT_SSH_PORT
        echo -e "${YELLOW}→ 使用当前 SSH 端口：${SSH_PORT}${NC}"
    else
        SSH_PORT=$CUSTOM_SSH
    ...
    if command -v ufw >/dev/null 2>&1; then
        ...
        echo -e "${GREEN}${ICON_SUCCESS} 已开放 SSH 端口 ${SSH_PORT}${NC}"
    fi
    echo ""
}
...
main_install() {
    ...
    echo -e "${GREEN}${ICON_SUCCESS} 已开放端口：SSH(${SSH_PORT}), Xray(${PORT})${NC}\n"
...
EOF
	...
	# ================== 自定义节点别名 ==================
	echo -e "${CYAN}${BOLD}请输入节点别名前缀 [留空回车 = 默认使用 Dack]：${NC}"
	read -r CUSTOM_NAME

	if [[ -z "$CUSTOM_NAME" ]]; then
		NODE_NAME="Dack-Reality_${NODE_IP}"
		echo -e "${YELLOW}→ 使用默认别名：${NODE_NAME}${NC}"
	else
		NODE_NAME="${CUSTOM_NAME}-Reality_${NODE_IP}"
		echo -e "${GREEN}→ 使用自定义别名：${NODE_NAME}${NC}"
	fi

	VLESS_LINK="vless://${UUID}@${NODE_IP}:${PORT}?encryption=none&flow=xtls-rprx-vision&security=reality&sni=${DOMAIN}&fp=chrome&pbk=${REALITY_PUBLIC_KEY}&sid=${VISION_SHORT_ID}&type=tcp&headerType=none#${NODE_NAME}"
